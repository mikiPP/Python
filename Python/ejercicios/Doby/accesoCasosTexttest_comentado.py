...
def accesoCasosTexttest(matrizCasosTest, rutaAccesoFichero):
    ...
    try:
        ...
        convertirStringsAIntegers(matrizCasosTest)
        fichero.close()
        return matrizCasosTest


def convertirStringsAIntegers(matrizCasosTest):
    for casosTestDia in matrizCasosTest:
        for stuff in casosTestDia:
            for (offset, article) in enumerate(stuff):
                if offset > 0:
                    stuff[offset]=int(article)
    return matrizCasosTest
